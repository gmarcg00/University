...
function duplicaPalabra(j){
    if(hayPalabra(j)){
        for(var i=(j+4);i<(j+8);i++){
            document.getElementById("letra"+i).value=document.getElementById("letra"+(i-4)).value;
            document.getElementById("letra"+i).setAttribute("style","color:red;width:30px ; height: 30px ;text-transform: uppercase");
        }
    }
    
}

function hayPalabra(j){
    for(var i=j;i<j+4;i++){
